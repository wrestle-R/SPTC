...
function ResultList({ result, points, playerName, teamName }: { result: ActivityResult; points: readonly number[]; playerName: (id: string) => string; teamName: (id: string) => string }) {
  return <div className="space-y-2">{points.map((pointsForPlace, index) => { const winner = result.placements[String(index + 1)]; const lineup = result.kind === "relay" ? result.lineups?.[winner] ?? [] : []; const pointsEach = lineup.length ? pointsForPlace / lineup.length : pointsForPlace; return <div key={pointsForPlace} className="flex items-center justify-between gap-3 rounded-xl border bg-muted/10 p-3"><div className="flex min-w-0 items-center gap-3"><span className={`grid size-8 shrink-0 place-items-center rounded-full text-xs font-black ${index === 0 ? "bg-amber-500/20 text-amber-400" : "bg-muted text-muted-foreground"}`}>{index + 1}</span><div className="min-w-0"><p className="truncate font-semibold">{result.kind === "relay" ? teamName(winner) : playerName(winner)}</p>{lineup.length ? <p className="truncate text-xs text-muted-foreground">{lineup.map(playerName).join(" · ")}</p> : null}</div></div><div className="shrink-0 text-right"><p className="text-sm font-black tabular-nums">{pointsForPlace} pts total</p>{lineup.length > 1 ? <p className="text-xs font-medium text-muted-foreground">{formatPoints(pointsEach)} each</p> : null}</div></div>; })}</div>;
}
