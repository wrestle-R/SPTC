  ...
  return <div className="mx-auto flex w-full max-w-6xl flex-col gap-6">
    ...
    <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      {sport.events.map((event) => {
        ...
        return <button key={event.id} type="button" onClick={() => selectEvent(event.id)} className={`rounded-2xl border p-4 text-left transition-all ${active ? "border-primary bg-primary/10 shadow-sm ring-1 ring-primary/30" : "bg-card hover:border-primary/40 hover:bg-muted/30"}`}><div className="flex items-center justify-between gap-3"><span className={`grid size-10 place-items-center rounded-xl ${event.kind === "relay" ? "bg-violet-500/15 text-violet-400" : "bg-sky-500/15 text-sky-400"}`}><Medal className="size-5" /></span>{result ? <Badge variant="secondary"><Check className="size-3" />Saved</Badge> : <Badge variant="outline">To score</Badge>}</div><p className="mt-5 font-bold leading-snug">{event.name}</p><p className="mt-1 text-xs text-muted-foreground">{event.kind === "relay" ? "Flexible team lineup" : "Individual event"} · {event.points.join(" / ")} pts</p></button>;
      })}
    </section>

    <Card className="overflow-hidden shadow-none"><CardHeader className="border-b bg-muted/20"><div className="flex items-start justify-between gap-3"><div><CardTitle>{selectedEvent.name}</CardTitle><CardDescription>{selectedEvent.kind === "relay" ? "Add one or more players for each team, then record the finishing order. Placement points are divided evenly within each lineup." : "Choose the first, second, and third-place players."}</CardDescription></div><div className="flex shrink-0 items-center gap-2"><Badge variant={selectedEvent.kind === "relay" ? "secondary" : "outline"}>{selectedEvent.kind === "relay" ? "Team lineup" : "Individual"}</Badge>{fixture ? <Button type="button" variant="ghost" size="icon-sm" className="text-destructive hover:bg-destructive/10 hover:text-destructive" onClick={() => setDeletingFixture(true)} aria-label="Delete fixture"><Trash2 /></Button> : null}</div></div></CardHeader><CardContent className="space-y-6 p-4 sm:p-6">
...
function PointsStrip({ points }: { points: readonly number[] }) { return <div><div className="grid grid-cols-2 gap-2 sm:grid-cols-4">{points.map((pointsForPlace, index) => <div key={pointsForPlace} className={`rounded-xl border p-3 ${index === 0 ? "border-amber-400/30 bg-amber-500/10" : "bg-muted/20"}`}><p className="text-xs font-bold uppercase tracking-wider text-muted-foreground">{placeNames[index]}</p><p className="mt-1 text-xl font-black tabular-nums">{pointsForPlace}<span className="ml-1 text-xs font-medium text-muted-foreground">pts total</span></p></div>)}</div><p className="mt-2 text-xs text-muted-foreground">Each total is divided evenly among the players in that team’s lineup. Example: 50 points ÷ 2 players = 25 points each.</p></div>; }
...
function RelayFixtureSetup({ teams, players, lineups, setLineups, pending, onCreate }: { teams: Team[]; players: Player[]; lineups: Record<string, string[]>; setLineups: (next: Record<string, string[]>) => void; pending: boolean; onCreate: () => void }) {
  return <div className="space-y-5"><div><h3 className="font-bold">Build the team lineups</h3><p className="mt-1 text-sm text-muted-foreground">Add one player for an individual event or multiple players for a relay. Viewers will see these saved lineups.</p></div><div className="grid gap-3 md:grid-cols-2">{teams.map((team) => <RelayLineupCard key={team.id} team={team} players={players.filter((player) => player.teamId === team.id)} selected={lineups[team.id] ?? []} onChange={(next) => setLineups({ ...lineups, [team.id]: next })} />)}</div><div className="flex justify-end border-t pt-5"><Button type="button" size="lg" onClick={onCreate} disabled={pending}>{pending ? "Creating fixture" : "Save lineups & create fixture"}</Button></div></div>;
}
