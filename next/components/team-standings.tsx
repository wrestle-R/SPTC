...
export function TeamStandings() {
  ...
  const rows = S9_TEAMS.map((team, index) => {
    ...
    return {
      ...
      activities: (stored?.["womens-games"] ?? 0) + (stored?.["senior-kids"] ?? 0) + (stored?.["junior-kids"] ?? 0) + (stored?.relay ?? 0) + (stored?.quickEvents ?? 0) + (stored?.bonus ?? 0) + (stored?.adjustments ?? 0) + (stored?.timelyArrival ?? 0) + (stored?.earlyBird ?? 0),
