...
import { DEFAULT_SPORT_RULES, normalizeSportRules, type Player, type SportRules, type Team } from "@sports-fiesta/domain";
import { LoaderCircle, Save } from "lucide-react";
import { useState } from "react";
import { toast } from "sonner";
import { ContentSkeleton, DataError } from "@/components/data-state";
import { OrganizerQuickEvents } from "@/components/organizer-quick-events";
...
export function OrganizerSettings() {
  ...
  if (tournament.loading || teams.loading || players.loading) return <ContentSkeleton />;
  if (tournament.error || teams.error || players.error) return <DataError message={tournament.error ?? teams.error ?? players.error ?? "Unable to load settings."} retry={tournament.retry} />;
  ...
  return <div className="flex flex-col gap-6"><div><h1 className="text-2xl font-semibold">Settings</h1><p className="mt-1 text-sm text-muted-foreground">Tournament details and sport placement points.</p></div>
    ...
    <OrganizerQuickEvents teams={teams.data} players={players.data} />
  </div>;
}
