...
export function OrganizerOverview() {
  ...
  const standings = usePrivateDocument<OverallStandingDocument>("standings", "overall");
  const loading = matches.loading || teams.loading || players.loading || leaderboards.loading || activities.loading;
  const error = matches.error || teams.error || players.error || leaderboards.error || activities.error;
  ...
  const liveActivities = activities.data.filter((record): record is ActivityFixture => record.type === "activity-fixture" && record.status === "live");
  const liveCount = liveMatches.length + liveActivities.length;
  const values = { live: liveCount, fixtures: matches.data.length, teams: teams.data.length, players: players.data.length };
  const player = (id?: string | null) => players.data.find((entry) => entry.id === id);
  const team = (id: string) => teams.data.find((entry) => entry.id === id);
  const overallLeaders = crossSportLeaders(matches.data, activities.data);
  ...
  return (
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-6">
      <section className="relative overflow-hidden rounded-3xl bg-slate-950 px-5 py-6 text-white shadow-xl sm:px-8 sm:py-8">
        ...
        <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Button nativeButton={false} render={<Link href="/organizer/matches" />} className="bg-white text-slate-950 hover:bg-slate-100">Manage fixtures <ArrowRight data-icon="inline-end" /></Button>
          </div>
        </div>
      </section>
      ...
      <TeamPointsChart teams={teams.data} standings={standings.data} />
      ...
      <section className="grid gap-6 xl:grid-cols-[1.35fr_1fr]">
        <div>
          ...
          {liveCount ? <div className="grid gap-3">{liveMatches.slice(0, 3).map((match) => <LiveMatchCard key={match.id} match={match} team={team} />)}{liveActivities.map((fixture) => <LiveActivityCard key={fixture.id} fixture={fixture} />)}</div> : <Card className="border-dashed"><CardContent className="flex min-h-48 flex-col items-center justify-center text-center"><span className="mb-3 grid size-11 place-items-center rounded-full bg-muted"><Activity className="size-5 text-muted-foreground" /></span><p className="font-semibold">No live fixtures right now</p><p className="mt-1 max-w-sm text-sm text-muted-foreground">Start a match or special event and it will appear here in real time.</p></CardContent></Card>}
        </div>
        <div>
...
function LiveActivityCard({ fixture }: { fixture: ActivityFixture }) {
  ...
  return <Link href={`/organizer/sports/${fixture.sport}`} className="block rounded-2xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><Card className="group border-rose-500/30 bg-rose-500/5 transition-all hover:-translate-y-0.5 hover:border-rose-500/60 hover:shadow-md"><CardContent className="p-5"><div className="mb-4 flex items-center justify-between"><span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{sport?.label ?? "Special event"}</span><Badge variant="destructive"><span className="mr-1.5 size-1.5 animate-pulse rounded-full bg-current" />Live</Badge></div><p className="font-semibold">{event?.name ?? "Event in progress"}</p><div className="mt-4 flex items-center justify-between border-t pt-3 text-sm text-muted-foreground"><span>{event?.kind === "relay" ? "Relay lineup ready" : "Recording results"}</span><span className="font-medium text-primary">Open event <ArrowRight className="ml-1 inline size-3.5 transition-transform group-hover:translate-x-1" /></span></div></CardContent></Card></Link>;
}
...
function crossSportLeaders(matches: PublicMatch[], activities: ActivityRecord[]) {
  const totals = new Map<string, number>();
  const addNormalized = (scores: Map<string, number>) => {
...
function activityPerformanceScores(result: ActivityResult) {
  ...
  for (const [index, points] of event.points.entries()) {
    const winner = result.placements[String(index + 1)];
    if (!winner) continue;
    if (result.kind === "individual") scores.set(winner, points);
    else {
      const lineup = result.lineups?.[winner] ?? [];
      for (const playerId of lineup) scores.set(playerId, points / lineup.length);
    }
  }
  return scores;
}
