...
export default function HomeScreen() {
  const { colors } = useAppTheme(); const matches = usePublicCollection<PublicMatch>("matches"); const teams = usePublicCollection<PublicTeam>("teams"); const players = usePublicCollection<PublicPlayer>("players");
  ...
  if (matches.loading || teams.loading || players.loading) return <Screen title="Home"><StateView loading /></Screen>;
  return <Screen title="Home" subtitle="Tournament overview and match centre.">
    {matches.error || teams.error || players.error ? <StateView error={matches.error || teams.error || players.error} /> : null}
    <View style={styles.stats}>{[["Live", live], ["Matches", matches.data.length], ["Teams", teamRows.length], ["Players", players.data.length || 77]].map(([label, value]) => <Card key={label as string} style={styles.stat}><Text style={[styles.statValue, { color: colors.text }]}>{value}</Text><Text style={[styles.statLabel, { color: colors.muted }]}>{label}</Text></Card>)}</View>
