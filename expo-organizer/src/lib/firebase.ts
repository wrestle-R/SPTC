...
// Metro exposes this through Firebase's React Native conditional export, while its default typings omit it.
const getReactNativePersistence = (FirebaseAuth as unknown as {
  getReactNativePersistence: (storage: typeof AsyncStorage) => never;
}).getReactNativePersistence;
export const auth = (() => { try { return FirebaseAuth.initializeAuth(app, { persistence: getReactNativePersistence(AsyncStorage) }); } catch { return FirebaseAuth.getAuth(app); } })();
